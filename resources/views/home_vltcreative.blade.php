...
<div class="container-fluid" style="padding-left: 1px; padding-right: 1px;">
    ...
    <div class="card shadow-lg border-0 w-100">
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle text-center mb-0" style="min-width: 1300px;">
                    <thead>
                        <tr>
                            <th>No</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $task->pivot->revisi_catatan }}</td>
                            <td>
<div class="d-flex justify-content-center flex-nowrap gap-2">

    {{-- Tombol Isi Peran --}}
    <button class="btn btn-warning btn-sm"
        data-bs-toggle="modal" data-bs-target="#peranModal{{ $task->tugas_id }}"
        {{ $task->status === 'Tugas Selesai' ? 'disabled' : '' }}>
...
<form action="{{ route('updateTaskStatus', ['tugas_id' => $task->tugas_id, 'status' => 'Sedang Dikerjakan']) }}" method="POST">
    ...
    <button type="submit" class="btn btn-info btn-sm"
        {{ $task->pivot->status !== 'Belum Dikerjakan' ? 'disabled' : '' }}>
...
<button type="button" class="btn btn-primary btn-sm"
    {{
        (
            !$task->pivot->peran || $task->isDeadlinePassed ||
            (
    ...
    <i class="bi bi-check2-circle me-1"></i> Selesai
               ...
               <!-- Modal upload gdrive -->
<div class="modal fade" id="uploadModal{{ $task->tugas_id }}" tabindex="-1" aria-labelledby="uploadModalLabel{{ $task->tugas_id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadModalLabel{{ $task->tugas_id }}">Unggah Hasil Pekerjaan</h5>
