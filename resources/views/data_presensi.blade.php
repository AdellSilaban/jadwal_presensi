            ...
            <table class="table table-bordered table-hover shadow rounded" style="border-color: #dee2e6;">
                <thead style="background: linear-gradient(to right, #f0f0f0, #e0e0e0);">
                    <tr>
                        ...
                        <th scope="col"style="text-align: center;">Tanggal Acara</th>
                    ...
                    @if($filterApplied && $presensi->count())
                        ...
                        @foreach ($presensi as $p)
                            <tr>
                                ...
                                <td style="text-align: center;">{{ \Carbon\Carbon::parse($p->jadwal->tgl_jadwal)->format('d/m/Y') }}</td>
                                <td style="text-align: center;">{{ $p->jadwal->agenda }}</td>
                                <td style="text-align: center;">{{ $p->volunteer->nama }}</td>
                               <td style="text-align: center;">
                                    {{ \Carbon\Carbon::parse($p->check_in)->translatedFormat('d/m/Y, H:i') }}
                                </td>
                                <td style="text-align: center;">
                                    {{ \Carbon\Carbon::parse($p->check_out)->translatedFormat('d/m/Y, H:i') }}
                                </td>
                               ...
                               <td style="text-align: center;">
                                @if ($p->check_in && $p->check_out)
                                    @php
                                        $durasiDetik = \Carbon\Carbon::parse($p->check_in)->diffInSeconds(\Carbon\Carbon::parse($p->check_out));
                                        $durasiMenit = $durasiDetik / 60;
                                        $jamFinal = floor($durasiMenit / 60);
                                    ...
                                    {{ $jamFinal }} jam
                                    <br>
                                    <small class="text-muted">Asli: {{ gmdate('H:i:s', $durasiDetik) }}</small>
                                @else
